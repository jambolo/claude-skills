...
- **files_in_scope** — the contract that makes parallelism safe: the worker touches only
  these paths, and the supervisor rejects the step if anything else changed.
- **actions** — when `depends_on` is non-empty, the first action asserts a concrete
  ...
  behind the supervisor's own gate.
...
- **Acceptance must be executable and exact** — a command plus its precise expected result,
  no "verify it works".
