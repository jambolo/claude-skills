...
`skills-manifest.json` at the repo root lists every skill and agent in its `entries` array, each with its current version, path, and mandatory `kind` (`skill` or `agent`); it is kept in lockstep with the frontmatter and is what `skill-version-check` compares installed copies against.
